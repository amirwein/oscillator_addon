...
def find_mode2(Eint_2d, xgrid, ygrid):
    ...
    norm = np.sum(Eint_2d, axis=(0, 1)) * dx * dy
    ...
    # second moments
    sig_x2 = np.sum(Eint_2d * x2, axis=(0, 1)) * dx * dy / norm
    sig_y2 = np.sum(Eint_2d * y2, axis=(0, 1)) * dx * dy / norm

    return sig_x2, sig_y2


def find_fwhm_pwr(pwr_s,rad_s,rad_e,delt):
    '''
    Function to compute pulse duration (FWHM, tau), peak position, and power
    ...
    pwr_s : array-like
        Power profile.
    rad_s : float
        Radiation window start time.
    rad_e : float
        Radiation window end time.
    delt : float
        Time resolution/time-slice spacing.
    ...
    pwr_max : float
        Maximum power
    '''
    # Original (Ginger-3D) time axis
    time_ax = np.arange(rad_e - delt/2, rad_s - delt/2, -delt)
    
    # Dense interpolation grid
    xq = np.linspace(time_ax.min(), time_ax.max(), 10000)
    
    # Shape-preserving interpolation
    interp = PchipInterpolator(time_ax[::-1], pwr_s[::-1])
    yq = interp(xq)
    
    # Peak
    peakIndex = np.argmax(yq)
    ymax = yq[peakIndex]
    peakPos = xq[peakIndex]
    halfMax = ymax / 2

    # Boolean mask above half max
    above = yq >= halfMax

    # Find crossings
    crossings = np.diff(above.astype(int))
    i1 = np.where(crossings == 1)[0] + 1            # rising edge
    i2 = np.where(crossings == -1)[0]               # falling edge
    
    # Distances from peak to nearest crossings
    filt_l = np.min(np.abs(peakPos - xq[i1])) if i1.size > 0 else 0.0
    filt_r = np.min(np.abs(peakPos - xq[i2])) if i2.size > 0 else 0.0
    
    tau = filt_l + filt_r
    pwr_max = ymax

    return tau, peakPos, pwr_max
