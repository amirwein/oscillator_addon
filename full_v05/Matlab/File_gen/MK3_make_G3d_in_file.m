function MK3_make_G3d_in_file(filename,inp_struc)
%% Function file for making a GINGER3D "in" file for the UHM FEL Oscillator
...
fprintf(fileID, 'current = %4.4f\n',inp_struc.currentMax);    %Peak current
% current shape
...
%% ginger-3D manual notes
%!window ≡ nphoton × dt slice = (nslice_sim / nsidep) × (Lw × λs /λw )
%nslice_sim ≥32π[ nsidep /(16πρNw) ]
%deprecated variable nphoton, use nslice_sim.
%For short pulse and oscillator runs with nslice sim > nside + nsidep
%requiring good z resolution well below the field gain length together with
% reasonable frequency resolution of the gain spectral bandpass will 
% probably require nside ≥96
%full electron beam duration: (nside × dt slice) ≡(nside/nslice_sim)×window
%nsidep ≤Nw 
%nsidep ≥16πρNw
%%
% how many e-beam slices you need
fprintf(fileID, 'nside = %4d\n',inp_struc.eslices);
% how many radiation slices, should be a power of two
fprintf(fileID, 'nslice_sim = %4d\n',inp_struc.rslices);
fprintf(fileID, 'lshort=.t\n');%
% total # of Photon slices each elec beam slice will interact with
