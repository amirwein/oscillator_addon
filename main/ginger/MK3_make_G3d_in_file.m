function MK3_make_in_file(filename,inp_struc)
...
fprintf(fileID, 'current = %4.4f\n',inp_struc.currentMax);                                 %Peak current 30
fprintf(fileID, 'ebeam_pulse_shape=''gaussian''\n');  	                   % current shape parabolic
fprintf(fileID, 'ebeam_tbody=%1.4e\n',inp_struc.RMSwidth);  	                           % RMS electron beam temporal pulse width
%fprintf(fileID, 't_start_end_ebeam(1:2)=0.0 3e-12\n');
...
%requiring good z resolution well below the field gain length together with rea-
%sonable frequency resolution of the gain spectral bandpass will probably require nside ≥96
%full electron beam duration, namely nside ×dt slice ≡(nside/nslice_sim)×window
%nsidep ≤Nw 
%nsidep ≥16πρNw
