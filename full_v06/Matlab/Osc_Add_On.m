...
if ~exist('DISP','var'), DISP = 1; end

% Efficient storage
if ~exist('EFF','var'), EFF = 0; end
...
% Number of passes in the cavity
if ~exist('npass','var'), npass = 400; end

% Electron Jitter
if ~exist('jitterRMS','var'), jitterRMS = 0; end

% Cavity Desync     % desync units = 2 (change in cavity length) / slippage
if ~exist('cav_d','var'), cav_d = 0; end

% Electron Desync (ramping)                       % Given in units [desync]
if ~exist('d_ramp','var'), d_ramp = zeros(1,npass); end


if ~exist('inp_struc','var')
%% UHM Beam parameters
